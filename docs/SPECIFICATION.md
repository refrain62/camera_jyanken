...
## 3. AVATAR-001: オリジナルの女の子の 3D アバター (Three.js WebGL) 描画仕様
...
- **頭部・表情姿勢マッピング**:
  - `WIN` (女の子敗北): 頭部 X軸回転 +0.25 rad（うつむき）、目 Y軸スケール 0.2（目を細める悔しがり表情）
  - `LOSE` (女の子勝利): 頭部 Z軸回転 $\pm 0.08\text{ rad}$（首ふりダンス）、口回転 $\pi\text{ rad}$（大喜び笑顔）
