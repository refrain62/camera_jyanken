...
MediaPipe Tasks Vision (`@mediapipe/tasks-vision`) を使用してWebカメラのフレームデータをグラフィックカード（WebGL / WASM）上で解析し、手の21個の3次元ランドマーク座標を検出する。また Three.js WebGL レンダラーを使用した初音ミク風3Dアバター (`MikuAvatar3D`) がCPU対戦相手としてインタラクティブにアニメーション動作する。
...
graph TD
    subgraph Browser Client
        ...
        Logic --> |Gesture: ROCK / SCISSORS / PAPER| Engine[useJankenGame State Machine]
        
        KeyBoard[Keyboard Event Listener - Space Key] --> |Trigger Start/Retry| Engine
        
        Engine --> |Trigger Sound| Audio[Voice & Audio Synthesizer (SpeechSynthesis & AudioContext)]
        Engine --> |Update Stage & Result| Miku[MikuAvatar3D Component (Three.js WebGL)]
        Engine --> |Update UI State| ReactUI[React Components Layout]
        
        VideoTag --> |Video Frame| OverlayCanvas[WebcamCanvas Overlay Renderer]
...
│   │   ├── CpuHandDisplay.tsx # CPU側の手表示・3Dミクアバターコンテナ
│   │   ├── MikuAvatar3D.tsx   # Three.js 初音ミク風3Dアバター WebGL レンダラー
...
│   │   └── useJankenGame.ts   # じゃんけん進行・あいこ連続カウントダウン・タイマー制御
│   ├── types/                 # 型定義
│   │   └── janken.ts          # じゃんけんの状態・手・3Dアバター型の定義
│   ├── styles/                # CSS スタイル
│   │   └── globals.css        # モダンサイバーグラスモーフィズムデザイン
│   ├── App.tsx                # メインアプリケーションコンポーネント (Spaceキー監視)
│   └── main.tsx               # エントリーポイント
...
## 4. ゲームステートマシン仕様 (useJankenGame.ts)

### 状態遷移定義
| 現在の状態 | 遷移トリガー | 遷移後の状態 | 副作用 (Side Effects) |
|---|---|---|---|
| `IDLE` | `START_GAME` ボタン / `Space` キー | `COUNTDOWN_JAN` | カウントタイマー開始 ($t=0\text{ms}$), 音声再生「じゃん」 |
| `COUNTDOWN_JAN` | $t \ge 800\text{ms}$ | `COUNTDOWN_KEN` | 音声再生「けん」 |
| `COUNTDOWN_KEN` | $t \ge 800\text{ms}$ | `COUNTDOWN_PON` | 音声再生「ぽん！」 |
| `COUNTDOWN_PON` | $t \ge 600\text{ms}$ | `JUDGEMENT` | 最新解析フレームの Gesture 取得, CPUの手（0=ROCK, 1=SCISSORS, 2=PAPER）を確定 |
| `JUDGEMENT` | 勝敗判定完了 ($t = 300\text{ms}$) | `RESULT` | スコア更新（勝/負/引き分け、連勝数更新）、エフェクト発動 |
| `RESULT` (勝敗決着時) | $t \ge 3000\text{ms}$ / `Space` キー | `IDLE` / `COUNTDOWN_JAN` | 表示リセット、または次対戦の即時開始 |
| `RESULT` (引き分け `DRAW` 時) | $t \ge 1200\text{ms}$ | `COUNTDOWN_AIKO` | 音声再生「あいこで」 |
| `COUNTDOWN_AIKO` | $t \ge 800\text{ms}$ | `COUNTDOWN_SHO` | 音声再生「しょ！」 |
| `COUNTDOWN_SHO` | $t \ge 600\text{ms}$ | `JUDGEMENT` | 再判定およびCPU手決定 |

## 5. 音声・Web Audio 制御仕様 (soundService.ts)

- **AudioContext 状態要件**: `AudioContext.state === 'running'` (ユーザー操作時 `Space` キー/ボタン押下時に `resume()` を実行)
- **音声読み上げ仕様**:
  - 言語: `ja-JP`
  - ピッチ: 1.1 (`Utterance.pitch = 1.1`)
  - 速度: 1.25 (`Utterance.rate = 1.25`)
  - ボリューム: 1.0 (`Utterance.volume = 1.0`)
- **追加音声メソッド**: `speakAiko()` («あいこで…»), `speakSho()` («しょ！»)
- **Web Audio SE**: Synth Oscillator によるカウントダウンビープ音 ($440\text{Hz}$ / $523.25\text{Hz}$ / $587.33\text{Hz}$ / $880\text{Hz}$ sine/triangle wave)

## 6. 品質およびテスト基準
- **すべてのクラス・関数**: JSDoc コメントが付与され、引数・戻り値・事前条件・事後条件が記載されていること。
- **型定義の完全性**: `any` 型の使用は禁止。
- **リンター検証**: `npm run lint` コマンドでエラー 0件、警告 0件 を満たすこと。
- **自動テスト検証**: `npm run test` コマンドでテストケース全件合格を満たすこと。
