...
## 1. プロジェクト概要
Webカメラから取得したリアルタイム映像からユーザーの手の形状（21個の3次元ハンドランドマーク）を解析し、「じゃん・けん・ぽん！」およびあいこ時の「あいこで…しょ！」の音声を交えたカウントダウンアニメーション、Three.js WebGL による女の子の3Dアバターと同期して自動対戦を行うWebアプリケーションの開発プロジェクトである。
...
- **RESULT**: 判定結果（勝利 / 敗北 / 引き分け）をカメラ側の下部へ表示し、CPUアバターと手を隠さない状態。勝敗確定時は 3.0秒経過後に IDLE 状態へ遷移し、引き分け時は 1.2秒後に自動的に `COUNTDOWN_AIKO` へ自動遷移する。

### 2.3 女の子のVRMアバター描画仕様
- AVATAR-002: 顔・髪・体・指骨格を備えたVRMへ刷新。現行仕様はdocs/SPECIFICATION.md第3節を参照。従来の図形合成方式と固定回転角の仕様を置き換える。
...
4. **Phase 4: 女の子の 3D アバター (Three.js WebGL) の構築**
   - Three.js WebGL レンダラーを使用した `GirlAvatar3D` コンポーネントの実装。
   - リアルタイムアニメーション（呼吸・浮遊・髪の揺れ・瞬き・手ポーズ・表情変更）。
